...
def inicializa_conexao_mysql():
    try:
        ...
        return parametros.MYSQL_CONNECTION
...
def verifica_sqlite():
    global BANCO_SQLITE, PASTA_MAXSUPORT
    PASTA_MAXSUPORT = os.path.join(parametros.SCRIPT_PATH.split('\\')[0], parametros.SCRIPT_PATH.split('\\')[1])
    if BANCO_SQLITE == '':
        BANCO_SQLITE = os.path.join(PASTA_MAXSUPORT, 'dados', 'dados.db')
...
def criar_tarefa_minuto(nome_executavel):
    ...
    INTERVALO_MINUTOS = 1
    start_trigger = task_def.Triggers.Create(1)
    start_trigger.Repetition.Interval = f"PT{INTERVALO_MINUTOS}M"
    start_trigger.Repetition.Duration = ""
